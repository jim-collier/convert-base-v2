...
Written by Anthropic Claude Sonnet 4.6, 2026-05-06
...
FONT_SIZE       = 50
CELL_W, CELL_H  = 56, 40      # pixel cell size
LABEL_H         = 12           # height of codepoint label below cell
COLS            = 16           # columns in debug grid

# Pixel thresholds (tune here)
BLANK_THRESHOLD      = 15      # fewer dark pixels → blank
DARK_PIXEL_MAX       = 180     # luminance below this = "dark"
EDGE_MARGIN          = 0.1    # fraction of cell dimension that counts as touching edge
CENTER_H_TOLERANCE   = 0.25   # fraction of CELL_W
CENTER_V_TOLERANCE   = 0.25   # fraction of CELL_H
COLOR_SATURATION_MIN = 30      # per-channel diff to count as "colored" pixel
COLOR_PIXEL_THRESHOLD= 5       # more than this many colored pixels → emoji-like
...
def analyze_cell(cell_img):
	...
	margin_h = max(1, round(CELL_W * EDGE_MARGIN))
	margin_v = max(1, round(CELL_H * EDGE_MARGIN))
	touches_edge = (
...
def main():
	...
	if debug:
		label_font = ImageFont.truetype(LABEL_FONT_PATH, 9)
	...
	for i, c in enumerate(chars):
		...
		if debug:
			...
			# Always draw cell border so overflow is visible
			border_color = (255, 0, 0) if fails else (0, 180, 0)
			gdraw.rectangle([x0, y0, x0 + CELL_W - 1, y0 + CELL_H - 1],
				outline=border_color, width=1)
			gdraw.text((x0 + 1, y0 + CELL_H + 1), f"{cp:04X}",
