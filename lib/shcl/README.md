...
Upstream is a real Go module (`github.com/jim-collier/shcl/source/go`, tagged `source/go/v1.2.0`), so `go get` would work. The copy is still deliberate: it leaves this program at standard library plus its own source, with no `go.sum` and nothing to fetch at build time.
