...
// UnknownBaseError: a base name or alias that resolves to nothing.
...
// MissingMarkerError: the output base carries no marker for something the
// value needs - a sign, or a fractional part.
...
// MarkerDefaultError: the global default marker is also one of the base's
// digits, so the base cannot finalize until the marker is set to something
// else or disabled.
...
// RetiredTokenError: a symbol spec carried one of the retired in-spec marker
// tokens (neg= / dec= / pad=). Markers are set separately now.
