...
| 20260712 |      |     | 3 | 3 | 2 |   |          |    |           | Fix flaky integration fuzz tests in test.bash | Randomized, pre-existing (not from the packaging change). Symbol-fuzz sometimes feeds a degenerate value like a lone `-` and counts the correct "empty input" error as a failure (seen: hostname->62, src=[-]); the v1-compat 32c encode check also intermittently disagrees with v1b. Guard the generators against degenerate inputs, and pin/log the seed so a failure is reproducible.
<!--
| 2026     |      |     |   |   |   |   |          |    |           |
-->
...
| 20260711 |      | 2.7 | 3 | 3 | 3 | 3 | 20260711 | JC | 20260711  | demo gif: smooth scrolling and cursor, content rework | Pixel-smooth scroll and gliding cursor; opens with crockford and emoji bases, explicit --from/--to, full base list scrolls by at reading pace, byte-encode section piped from a real /bin binary; faster digit typing; 16-color aliased frames keep it near 2 MiB
