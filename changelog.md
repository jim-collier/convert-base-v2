...
- Converting a long number between two bases that are not powers of two is much faster and uses far less memory. A 16,000 digit conversion dropped from 251 to 2.8 milliseconds, and from 882 MB of working memory to about 1 MB. A 40,000 digit one dropped from 1.5 seconds to 0.07. Short numbers are unaffected in practice, but gain too.
