...
- Breaking: `2048tt` is gone, and the digits of `256tt`, `512tt`, and `1024tt` changed. Anything encoded in one of those four bases has to be decoded with the previous build before upgrading. The smaller members of the family, `32tt` through `128tt`, are untouched.
...
- A base between `512tt` and the published base 2048s: `1024tt`.
...
- `512tt` and `1024tt` now end a byte stream with a tail character, the same approach the published big bases use, which is what lets them stream. Their binary layout changed as a result. Neither has been in a release, so no existing data is affected.
- The digits of the `tt` family are now in ascending code point order. That changes what `256tt`, `512tt`, and `1024tt` write, since the first digit that moved sits inside the first 256. `32tt`, `64tt`, and `128tt` draw only from the part that did not move, so they are unchanged.
...
- Base `2048tt`. Its digits were not in code point order, and there was no way to correct that without leaving two incompatible alphabets under one name. The rest of the family covers the same ground up to `1024tt`, and the two published base 2048s remain.
